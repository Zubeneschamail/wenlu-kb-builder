...
- 常见源码和配置文本：Vue / JavaScript / TypeScript / HTML / CSS、C / C++ / C#、Java / Kotlin、Go / Rust / Python，以及 Shell / PowerShell / SQL / XML / YAML / TOML 等；按纯文本解析，保留相对路径和来源行范围，不做语法树分析。
...
- 每文件最多 25 MB、200 万提取字符，PDF 最多 200 页；每包最多 10 万个文档、100 万个片段、8 GiB，包内清单最多 64 MiB。大包需要配套更新后的闻录，旧安装版仍受原容量限制。
- 大项目逐文档解析并写入临时数据库，每批最多生成 64 个不同文本的向量；旧向量从上次知识包逐条复用。失败或取消保留旧包，输出磁盘需留出旧包、新包与临时索引共存的空间。检索每批扫描 512 个向量，仅加载最终命中的正文；检索时间仍随片段总数增长。
- 空文档会在文档清单中记录跳过原因；损坏或无法读取的文档仍会阻止发布。不支持的文件只保留前 1000 条跳过详情，其余汇总数量，避免代码项目的排除记录撑大清单。源码按文本切块并建立向量，不会编译、执行或自动补全代码。
- 文件夹默认跳过隐藏目录、evaluation、tests、models、output、build、dist、bin、obj、coverage、target、依赖目录和符号链接。其他不支持文件记录到报告。显式选择的单个文件仍需由操作者确认属于客户资料。
